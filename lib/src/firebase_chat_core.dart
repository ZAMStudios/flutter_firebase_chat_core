...
class FirebaseChatCore {
  ...
  Future<types.Room> createRoom(
    ...
    final rooms = await processRoomsQuery(fu, query, otherUser);

    try {
      return rooms.firstWhere((room) {
        print('room is ${room.id}');
        if (room.type == types.RoomType.group) return false;
        final userIds = room.users.map((u) {
          return u.id;
        });
        return userIds.contains(fu.uid) &&
            userIds.contains(otherUser.firebaseId);
      });
    } catch (e) {
  ...
  Stream<types.Room> room(String roomId, ReviewUser.User otherUser) {
    final fu = firebaseUser;
    ...
    return FirebaseFirestore.instance
        ...
        .snapshots()
        .asyncMap(
          (doc) => processRoomDocument(doc, fu, otherUser),
    );
  }
  ...
  Stream<List<types.Room>> rooms(
      {bool orderByUpdatedAt = false, required ReviewUser.User otherUser}) {
    final fu = firebaseUser;
    ...
    return collection.snapshots().asyncMap(
          (query) => processRoomsQuery(fu, query, otherUser),
    );
  }
